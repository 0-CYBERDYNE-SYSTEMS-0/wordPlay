...
function createToolBasedPrompt(instruction: string, textContext: string, selectionInfo: any, customGuidance?: string, enhancedContext?: any): string {
...
EXAMPLE RESPONSE:
<thinking>
I need to analyze this text and improve its clarity. The current sentence structure is overly complex and could be simplified. I'll focus on breaking down long sentences and removing unnecessary jargon.
</thinking>
...
async function analyzeContentAndContext(content: string, includeContext: boolean = false, projectId?: number): Promise<{
  ...
  // If context is requested, fetch project sources from database
  ...
  if (includeContext && projectId) {
    try {
      ...
      if (projectSources.length > 0) {
        ...
        // Prepare research content for AI
